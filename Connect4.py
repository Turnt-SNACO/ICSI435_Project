...
def winning(board, piece):
    for i in range(Num_Col-3): #last 3 is not gonna work (vertical)
        for j in range(Num_Row):
            if board[j][i] == piece and board[j][i+1] == piece and board[j][i+2] and board[j][i+3] == piece:
                return True
    
    for i in range(Num_Col): #last 3 is not gonna work (horizontal)
        for j in range(Num_Row-3):
            if board[j][i] == piece and board[j+1][i] == piece and board[j+2][i] and board[j+3][i] == piece:
                return True
            
    for i in range(Num_Col-3): #last 3 is not gonna work (positive diagonal)
        for j in range(Num_Row-3):
            if board[j][i] == piece and board[j+1][i+1] == piece and board[j+2][i+2] and board[j+3][i+3] == piece:
                return True
